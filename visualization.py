...
def flow_to_rgb(
    ...
    float_hue = (
        wheel[angle_floor.astype(np.int32)] * (1 - angle_fractional) + wheel[angle_ceil.astype(np.int32)\
                                                                            ] * angle_fractional
    )
    ...
    if background == "dark":
        ...
        parameters = ColorizationArgs(move_hue_on_S_axis, move_hue_on_V_axis, np.array([0, 0, 0\
                                                                                       ], dtype=np.float32))
...
def custom_draw_geometry_with_key_callback(pcds):
    def change_background_to_real_black(vis):
    ...
    def capture_image(vis):
        ...
        return False
    key_to_callback = {}
    key_to_callback[ord("B")] = change_background_to_real_black
...
def show_flows(pc1, pc2, flow):
    ...
    colors_flow = flow_to_rgb(flow)
    pc1_o3d_gt.points = o3d.utility.Vector3dVector(pc1)
